...
const BuyHomesPage: React.FC<BuyHomesPageProps> = ({ onPropertyClick }) => {
  ...
  const buyPageContent = content.buyHomesPage;
  // Filter properties to show only 'buy' type
  const propertiesToDisplay = properties.filter(p => p.type === 'buy');

  if (isLoading) {
    return <main className="flex-1 flex flex-col items-center py-16 text-xl dark:text-white">Loading properties for sale...</main>;
  }
    ...
    <main className="flex-1 flex flex-col items-center">
      <div className="max-w-[1280px] w-full px-4 md:px-10 lg:px-40 py-8">
        ...
        <div className="flex gap-3 pb-6 flex-wrap sticky top-[68px] z-40 bg-background-light/95 dark:bg-background-dark/95 backdrop-blur-sm py-2">
          {buyPageContent.filters.map(filter => (
          ...
          <div className="flex flex-1 flex-col items-start justify-between gap-4 rounded-xl border border-primary/20 bg-primary/5 p-5 sm:flex-row sm:items-center">
            <div className="flex items-center gap-4">
              <div className="flex items-center justify-center size-10 rounded-full bg-primary text-[#141811]">
                <span className="material-symbols-outlined">{buyPageContent.trustStrip.icon}</span>
              </div>
              <div className="flex flex-col gap-0.5">
                <p className="text-[#141811] dark:text-white text-base font-bold leading-tight">
                  {buyPageContent.trustStrip.heading}
                </p>
                <p className="text-[#758961] dark:text-[#a5b694] text-sm font-normal leading-normal">
                  {buyPageContent.trustStrip.description}
                </p>
              </div>
            </div>
            <a className="text-sm font-bold leading-normal tracking-[0.015em] flex items-center gap-2 text-[#141811] dark:text-primary hover:underline" href={buyPageContent.trustStrip.linkUrl}>
              {buyPageContent.trustStrip.linkText}
              <span className="material-symbols-outlined text-[20px]">{buyPageContent.trustStrip.linkIcon}</span>
            </a>
          </div>
        </div>
        {/* Listings Grid */}
        ...
        <div className="flex justify-center mt-12 mb-20">
          <button className="flex items-center gap-2 px-8 py-3 rounded-lg border border-[#e0e6db] dark:border-[#3a4830] font-bold hover:bg-[#f2f4f0] dark:hover:bg-[#2a3620] transition-all">
            <span>{buyPageContent.loadMoreButtonText}</span>
            <span className="material-symbols-outlined">{buyPageContent.loadMoreButtonIcon}</span>
          </button>
        </div>
      </div>
    </main>
